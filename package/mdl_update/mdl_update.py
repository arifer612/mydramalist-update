...
def wiki(title): # Searches for wikipedia link. Search term has to be accurate
    query = title + 'site:ja.wikipedia.org'
    wiki_link = [link for link in search(query, lang='jp', start=0, stop=1, pause=0.1)][0]
    return wiki_link


def title_search(title): # Searches for MDL link. Search term has to be accurate
    res = requests.get('https://mydramalist.com/search?q={}'.format(title))
    soup = bs4.BeautifulSoup(res.text, 'lxml')
    link = 'https://mydramalist.com{}'.format(soup.find('h6').contents[0]['href'])
    root = '{}/episode/'.format(link)
    res = requests.get(link)
    soup = bs4.BeautifulSoup(res.text,'lxml')
    native_title = soup.find('b',text='Native Title:').next_sibling.next_sibling.text
    wiki_link = wiki(native_title)
    return root, wiki_link


def episodes(title): # Generates a list with the episode numbers and its air dates
    root, wiki_link = title_search(title)
